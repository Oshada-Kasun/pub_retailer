var fsBanner = function(container,options) {
	...
	this.setup = function() {
		...
		this.itemsifrm = this.container.find('iframe');
		...
		this.itemsifrm.css({'height':this.container.height(),'width':this.widmain+this.mini});

		if (!this.options.showName) {
			this.items.find('.name').hide();
			this.itemsifrm.find('.name').hide();
		}
		...
		this.itemsifrm.each(function(i) {
			var $itemifrm = $(this);
			$itemifrm.css({'z-index':i});
			if (self.options.trigger == 'click') $itemifrm.on('click',function() { self.selectItem($itemifrm,i); });
			if (self.options.trigger == 'mouse') $itemifrm.on('mouseenter',function() { self.selectItem($itemifrm,i,true); });
		});
